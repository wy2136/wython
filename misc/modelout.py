...
def get_modelout_data(daname, model, expname, func=None, funcname='processed', ofile=None, loop_files=None, verbose=True, precheck=False, savedata=True, **kws):
    ...
    if loop_files:
        ...
        for ii,ifile in enumerate(ifiles, start=1):
            print(f'[{ii:03d} of {nfiles:03d}]: {ifile}')
...
def get_modelout_data_ens(daname, model, expname, ens, func=None, funcname='processed', ofile=None, **kws):
    """get model data given daname, model, expname, ens, func and parameters from get_modelout_data."""
    if func is None:
        func = lambda x: x
    nens = len(list(ens))
    #ofile
    ...
    if os.path.exists(ofile):
        ...
        for en in ens:
            ...
            da = get_modelout_data(daname, model, expname, func=func, funcname=funcname, savedata=False, ens=en, **kws)
            das.append(da)
