...
database = {
    'iod': {
        'long_name': 'Indian Ocean Dipole mode index',
        'url': 'https://www.esrl.noaa.gov/psd/gcos_wgsp/Timeseries/Data/dmi.long.data',
        'invalid_value': -99.99,
    },
    'ipo': {
        'long_name': 'Interdecadal Pacific Oscillation',
        'url': 'https://www.esrl.noaa.gov/psd/data/timeseries/IPOTPI/tpi.timeseries.ersstv5.data',
        'invalid_value': '-99.000',
    },
    'soi': {
    ...
    'amo_sm': {
        ...
        'url': 'https://www.esrl.noaa.gov/psd/data/correlation/amon.sm.long.data',
...
def _get_year_start_end(climate_index_name):
    ...
    r = requests.get(url)
    df = pd.read_csv(io.StringIO(r.text), header=None, nrows=1, sep=r'\s+')
...
def _read_csv(climate_index_name):
    ...
    if year_start < 1678:# ignore records before 1678 since pandas is not able to handle this
        ...
        Nskiprecords = 0
    r = requests.get(url)
    df = pd.read_csv(io.StringIO(r.text), header=None, skiprows=1+Nskiprecords,
