...
from xtc import tc_tracks, tc_basins, tc_count, tc_density

expname = 'CTL1860_newdiag_tigercpu_intelmpi_18_576PE'
years = range(1, 1001)
thisjob = f'{expname}, years {years[0]:04d}-{years[-1]:04d}'

# TC tracks
ofile = f'tc_tracks.{years[0]:04d}-{years[-1]:04d}.nc'
if not os.path.exists(ofile):
    ds = tc_tracks(expname=expname, years=years)
...
ofile = f'tc_counts.{years[0]:04d}-{years[-1]:04d}.nc'
if not os.path.exists(ofile):
...
ofile = f'tc_ace.{years[0]:04d}-{years[-1]:04d}.nc'
if not os.path.exists(ofile):
    basins = ('global', 'NA', 'EP', 'WP', 'NI', 'SI', 'AU', 'SP', 'SA')
...
if not os.path.exists(ofile_yearly):
    ...
    ds_ace_yearly = ds_ace.groupby('time.year').sum('time')
    encoding = None
...
# TC density
ofile = f'tc_density.{years[0]:04d}-{years[-1]:04d}.nc'
if not os.path.exists(ofile):
